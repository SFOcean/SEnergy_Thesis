#include <Arduino.h>
#include <WiFiClientSecure.h>
#include "PubSubClient.h"

#include "ZMPT101B.h"
//ZMPT101B voltageSensor(signalPin1);
int voltagePin = 32;
int signalPin1 = 33;
int signalPin2 = 34;
int signalPin3 = 35;

ZMPT101B voltageSensor0(voltagePin);
ZMPT101B voltageSensor1(signalPin1);
ZMPT101B voltageSensor2(signalPin2);
ZMPT101B voltageSensor3(signalPin3);

float crnt1, crnt2, crnt3, crnt4; // variables to send 

float dcOffsetSamples = 0;

float analogBuffer[400];

float ADCScale = 4095.0;
float Vref = 5.0;
float sensitivity = 0.00527;


#define WIFI_NETWORK ""
#define WIFI_PASSWORD ""
#define WIFI_TIMEOUT_MS 20000

const char* mqtt_server = "f2bed6f24e6244468bf22c01d7e50881.s2.eu.hivemq.cloud";
const char* username = "easha";
const char* password  = "easha1700";
const int port = 8883;

//root certificate
static const char *root_ca PROGMEM = R"EOF(
-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----
-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----
-----BEGIN CERTIFICATE-----

Dfvp7OOGAN6dEOM4+qR9sdjoSYKEBpsr6GtPAQw4dy753ec5
-----END CERTIFICATE-----
)EOF";



void publishMessage(const char *topic);
void callback(char *topic, byte *payload, unsigned int length);

WiFiClientSecure espClient;
PubSubClient client(mqtt_server,port,callback,espClient);


unsigned long lastMsg = 0;
unsigned long value = 0;

void connectToWifi() {
  Serial.print("Connecting to WiFi");
  WiFi.mode(WIFI_STA);
  WiFi.begin(WIFI_NETWORK, WIFI_PASSWORD);

  unsigned long startAttemptTime = millis();

  while(WiFi.status() != WL_CONNECTED && millis() - startAttemptTime < WIFI_TIMEOUT_MS){
    Serial.print(".");
    delay(100);
  }

  if(WiFi.status() != WL_CONNECTED) {
    Serial.print("Failed!");
  
  }else {
    Serial.println(" ");
    Serial.print("Connected ");
    Serial.println(WiFi.localIP());
  }
}


void reconnect() {
  // Loop until we're reconnected
  while (!client.connected()) {
    Serial.print("Attempting MQTT connection...");
    // Attempt to connect
    if (client.connect("arduinoClient",username,password)) {
      Serial.println("connected to  mq");
      // Once connected, publish an announcement...
      client.publish("testconnect","hello world");
      // ... and resubscribe
      client.subscribe("inTopic");
    } else {
      Serial.print("failed, rc=");
      Serial.print(client.state());
      Serial.println(" try again in 5 seconds");
      // Wait 5 seconds before retrying
      delay(5000);
    }
    
  }
}

void setup() {
  // put your setup code here, to run once:
  //delay (1000);
  Serial.begin (115200);
  pinMode (signalPin1, INPUT);
  pinMode (signalPin2, INPUT);
  pinMode (signalPin3, INPUT);
  pinMode (15, OUTPUT);
  pinMode (16, OUTPUT);
  pinMode (17, OUTPUT);
  connectToWifi();
  espClient.setCACert(root_ca);
 
  delay(1500);
}

void loop() {
  // put your main code here, to run repeatedly:

  digitalWrite (15, HIGH);
  digitalWrite (16, HIGH);
  digitalWrite (17, HIGH);
  // For Voltage
  float accum = 0;
  for (int i=0; i<400; i++)
  {
    accum += analogRead (voltagePin);
    delayMicroseconds(250);
  }
  dcOffsetSamples = accum/400;
  voltageSensor0.setZeroPoint(dcOffsetSamples);
  voltageSensor0.setSensitivity (sensitivity);
  float vrms0 = voltageSensor0.getVoltageAC(50);
  Serial.print ("         ");
  crnt1 = vrms0*0.6055;
  Serial.print (crnt1);

  
  // For signnalPin 1
  
  accum = 0;
  for (int i=0; i<400; i++)
  {
    accum += analogRead (signalPin1);
    delayMicroseconds(250);
  }
  dcOffsetSamples = accum/400;
  voltageSensor1.setZeroPoint(dcOffsetSamples);
  voltageSensor1.setSensitivity (sensitivity);
  float vrms1 = voltageSensor1.getVoltageAC(50);
  Serial.print ("         ");
  crnt2 = vrms1*0.37;
  Serial.print (crnt2);

  // For signalPin2
// voltageSensor(signalPin2);
   accum = 0;
    for (int i=0; i<400; i++)
    {
      accum += analogRead (signalPin2);
      delayMicroseconds(250);
    }
    dcOffsetSamples = accum/400;
    voltageSensor2.setZeroPoint(dcOffsetSamples);
    voltageSensor2.setSensitivity (sensitivity);
    float vrms2 = voltageSensor2.getVoltageAC(50);
    Serial.print ("         ");
    crnt3 = vrms2*0.37;
    Serial.print (crnt3);

   // For signalPin3
   //ZMPT101B voltageSensor(signalPin3);
   accum = 0;
    for (int i=0; i<400; i++)
    {
      accum += analogRead (signalPin3);
      delayMicroseconds(250);
    }
    dcOffsetSamples = accum/400;
    voltageSensor3.setZeroPoint(dcOffsetSamples);
    voltageSensor3.setSensitivity (sensitivity);
    float vrms3 = voltageSensor3.getVoltageAC(50);
    Serial.print ("         ");
    crnt4 = vrms3*0.37;
    Serial.println (crnt4);
  delay(1000);  


  //Server Code Omor  
   if (!client.connected()) {
    reconnect();
  }
  client.loop();

  unsigned long now = millis();
  if(now - lastMsg > 5000){
    lastMsg = now;
    
   publishMessage("currentValue");
  }
}


void publishMessage(const char* topic){
  String json = buildJson();
  char jsonStr[200];
  json.toCharArray(jsonStr,200);
  client.publish(topic, jsonStr);
}


void callback(char* topic, byte* payload, unsigned int length) {
  Serial.print("Message arrived [");
  Serial.print(topic);
  Serial.print("] ");
  for (int i=0;i<length;i++) {
    Serial.print((char)payload[i]);
  }
  Serial.println();
}

 String buildJson() {
   String data = "{";
   data+="\n";
   data+= "\"Data\": {";
   data+="\n";
   data+="\"topicName\": \"CurrentValue \",";

   data+="\n";
   data+="\"current 1\": ";
   data+=crnt1;
data+= ",";

   data+="\n";
   data+="\"current 2\": ";
   data+=crnt2;
data+= ",";

  data+="\n";
   data+="\"current 3\": ";
  data+=crnt3;
data+= ",";

   data+="\n";
   data+="\"current 4\": ";
  data+=crnt4;

    data+="\n";
   data+="}";
   data+="\n";
   data+="}";
   
   return data;
  
 }
